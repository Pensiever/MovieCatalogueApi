...
{
    public class PersonRepository : BaseRepository, IPersonRepository<Person, Cast>
    {
        ...
        public IEnumerable<Cast> GetMovieByPersonId(int Id)
        {
            using (SqlConnection c = Connection())
            {
                c.Open();
                using (SqlCommand cmd = c.CreateCommand())
                {
                    ...
                    cmd.CommandText = $"SELECT a.Role, m.Title, a.MovieID FROM Movie m JOIN Actor a ON a.MovieID = m.Id WHERE a.PersonID = @id ";
                    cmd.Parameters.AddWithValue("id", Id);
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
